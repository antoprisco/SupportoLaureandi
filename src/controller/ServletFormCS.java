...
public class ServletFormCS extends HttpServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (Integer.parseInt(request.getParameter("flag")) == 8) {
      ...
      if (request.getParameter("nome").length() == 0 || request.getParameter("nome").length() > 20
      ...
      Residenza r = new Residenza(request.getParameter("residenza"),
          request.getParameter("provinciaR"), request.getParameter("indirizzo"),
          Integer.parseInt(request.getParameter("cap")));
      TitoloStudio ts = new TitoloStudio(request.getParameter("laurea"),
          request.getParameter("dataL"), request.getParameter("universita"),
      ...
      RequestCS rcs1 = new RequestCS(request.getParameter("nome"), request.getParameter("cognome"),
          1);
      ...
      try {
        ...
        // ArrayList<RequestCS> list6 = rcD.doRetrieveByNCS(user.getName(),
        // user.getSurname(), 6);
        ...
        if (list1.isEmpty() && list2.isEmpty() && list3.isEmpty() && list4.isEmpty()
          ...
          try {
            Date d = new Date();
            nome_file = new File(
                "IscrizioneCS_" + user.getSurname() + d.getHours() + "" + d.getMinutes() + ".pdf");
